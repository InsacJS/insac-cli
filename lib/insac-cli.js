...
program.command('new <app> [description]').action((app, description) => {
  ...
  let modulePath             = path.resolve(process.cwd(), APP_NAME)
  ...
  util.copyDir(source, modulePath)
  ...
  util.mkdir(path.resolve(modulePath, 'src/modules'))
  try { util.cmd('git init', modulePath) } catch (e) {}
