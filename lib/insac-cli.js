...
program.command('new <app> [description]').action(async (app, description) => {
  try {
    ...
    let indexSourcePath         = path.resolve(__dirname, './templates/index.html')
    ...
    let indexDestinationPath    = path.resolve(process.cwd(), `./${APP_NAME}/public/index.html`)
    ...
    let index        = util.readFile(indexSourcePath)
    ...
    indexContent = indexContent.replace('{{APP_NAME}}', _.upperFirst(_.replace(APP_NAME, /_/g, ' ')))
    indexContent = indexContent.replace('{{INSAC_VERSION}}', insacVersion())
    indexContent = indexContent.replace('{{CLI_VERSION}}', cliVersion())
    util.writeFile(packageDestinationPath,  `${JSON.stringify(pack,     null, 2)}\n`)
    util.writeFile(readmeDestinationPath, readmeContent)
    util.writeFile(indexDestinationPath, indexContent)
    util.writeFile(gitignoreDestinationPath, gitignore)
    util.mkdir(path.resolve(projectPath, 'src/modules'))
    const task = new Loader(' - Instalando dependencias')
    try {
      console.log()
      task.start()
    ...
    try { await util.cmd('git init', projectPath) } catch (e) {}
...
program.command('add:module <name>').action((name) => {
  ...
  if (!['MODULE', 'RESOURCE', 'SENDGRID_MAIL'].includes(MODULE_TYPE)) {
    throwError(`El tipo de módulo '${MODULE_NAME}' no existe. Usa 'insac --info' para obtener más información.`)
  }
  ...
  let moduleFileDestinationPath = path.resolve(modulePath, `${MODULE_NAME.toLowerCase()}.module.js`)
...
function insacVersion () {
  ...
  return require(packageProjectPath).dependencies.insac.substr(1)
}
