...
program.command('new <app> [description]').action(async (app, description) => {
  try {
    ...
    let indexTargetPath        = path.resolve(process.cwd(), `./${APP_NAME}/public/index.html`)
    ...
    try {
      ...
      task.start()
      process.stdout.write(await util.cmd(`cd ${APP_NAME} && npm install`, process.cwd()))
      task.finish('\n')
    } catch (e) { task.failed('\n') }
    let indexContent = index
    indexContent = indexContent.replace('{{APP_NAME}}', _.upperFirst(_.replace(APP_NAME, /_/g, ' ')))
    indexContent = indexContent.replace('{{INSAC_VERSION}}', insacVersion(projectPath))
    indexContent = indexContent.replace('{{CLI_VERSION}}', cliVersion())
    util.writeFile(indexTargetPath, indexContent)
