...
program.command('add:module <name>').action((name) => {
  ...
  if (MODULE_TYPE === 'RESOURCE') {
    ...
    util.mkdir(path.resolve(modulePath, `seeders/production`))
  }
  ...
  return new Promise((resolve, reject) => {
    getApp().then(app => {
      ...
      if (resourceModules.length === 1) {
        ...
        return throwError(`Se requiere el nombre de un módulo de tipo RESOURCE (ejemplo: <comando> -M API). Módulos: ${resourceModules.toString()}`)
      }
    }).catch(e => { reject(e) })
  })
}
