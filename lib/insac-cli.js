...
program.option('-l, --level <value>',    'Nivel de submodelos a incluir en el OUTPUT.')
...
program.command('add:module <name>').action((name) => {
  ...
  ncp(source, destination, function (err) {
    ...
    if (!exist(configContent, `service.addModule('${name}')`)) {
...
program.command('add:output <group>').action((group) => {
  if (!group)          { throwError(`Se requiere el grupo al que pertenece (directorio).`) }
  if (!program.module) { throwError(`Se requiere el nombre del módulo.`) }
  if (!program.model)  { throwError(`Se requiere el nombre del modelo.`) }
  ...
  createDATA(MODULE_NAME, MODEL_NAME).then(data => {
    ...
    outputContent = outputContent.replace(ROUTE_FLAG, newOutputContent)
    util.writeFile(outputPath, outputContent)
    process.stdout.write(`\x1b[32m\n Componente \x1b[0m${GROUP}/${MODEL_NAME}.output.js/${KEY}\x1b[32m creado exitosamente\x1b[0m \u2713\n\n`)
  }).catch((err) => {
    console.log(err)
    process.stdout.write(`Hubo un problema al cargar la aplicación, verifique e inténtelo nuevamente.\n`)
    process.exit(0)
...
function help () {
  ...
  Modo de uso: insac [comando] [opcion]
    ...
    Opciones:
      ...
      -l, --level <value>       Nivel de submodelos a incluir en el OUTPUT.
    ...
    Ejemplos:
        ...
        insac add:output api/v1/inscripciones -k listar -M central -m inscripcion -l 3\n\n`
