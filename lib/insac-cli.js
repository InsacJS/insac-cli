...
program.command('new <app> [description]').action((app, description) => {
  ...
  // let packLockSourcePath      = path.resolve(__dirname, './templates/package-lock.json')
  ...
  // let packLockDestinationPath = path.resolve(process.cwd(), `./${app}/package-lock.json`)
  let yarnLockDestinationPath = path.resolve(process.cwd(), `./${app}/yarn.lock`)
  let pack         = require(packageSourcePath)
  // let packLock     = require(packLockSourcePath)
  ...
  ncp(source, destination, function (err) {
    ...
    // util.writeFile(packLockDestinationPath, `${JSON.stringify(packLock, null, 2)}\n`)
