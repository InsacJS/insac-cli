...
program.command('new <app> [description]').action((app, description) => {
  ...
  util.mkdir(path.resolve(destination, 'src/modules'))
...
program.command('add:module <name>').action((name) => {
  ...
  let destination    = path.resolve(MODULES_PATH, MODULE_NAME)
  if (!util.isDir(MODULES_PATH)) { util.mkdir(MODULES_PATH) }
  if (util.isDir(destination) && !FORCE) {
    throwError(`El módulo ${MODULE_NAME} ya existe.\nDirectorio: ${destination}`)
  }
  if (MODULE_TYPE === 'RESOURCE') {
    util.mkdir(path.resolve(MODULES_PATH, `${MODULE_NAME}/dao`))
    util.mkdir(path.resolve(MODULES_PATH, `${MODULE_NAME}/models`))
    util.mkdir(path.resolve(MODULES_PATH, `${MODULE_NAME}/resources`))
    util.mkdir(path.resolve(MODULES_PATH, `${MODULE_NAME}/seeders`))
    util.mkdir(path.resolve(MODULES_PATH, `${MODULE_NAME}/seeders/prod`))
  }
  let moduleFileSourcePath      = path.resolve(__dirname, `./templates/modules/${MODULE_TYPE}/example.module.js`)
  let moduleFileDestinationPath = path.resolve(MODULES_PATH, `${MODULE_NAME}/${MODULE_NAME.toLowerCase()}.module.js`)
  util.copyDir(source, destination)
...
function help () {
  ...
  Modo de uso: insac [comando] [opciones]
    ...
    Ejemplos:
      ...
      insac add:route listar -r api/v2/libros
      insac add:route welcome -r api/v2/custom
