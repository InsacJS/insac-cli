...
module.exports = (data) => {
  ...
  Object.keys(MODEL.attributes).forEach(key => {
    ...
    if (!isAudit) {
      fields.push({ name: key, example: _getExample(FIELD) })
...
function _getExample (field) {
  if (typeof field.example !== 'undefined') {
    return (typeof field.example === 'string') ? `'${field.example}'` : (typeof field.example === 'object' ? _.replace(JSON.stringify(field.example), /"/g, '\'') : `${field.example}`)
  }
