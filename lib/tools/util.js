...
*/
exports.copyDir = (source, target, logger, workspacePath) => {
  function _copy (sourcePath, targetPath) {
    const relativeTargetPath = targetPath.replace(workspacePath, '')
    if (fs.statSync(sourcePath).isDirectory()) {
      mkdirp.sync(targetPath)
      logger.appVerbose('[carpeta]', `${relativeTargetPath} ${logger.OK}`)
      ...
      logger.appVerbose('[archivo]', `${relativeTargetPath} ${logger.OK}`)
    }
  }
  _copy(source, target)
}
