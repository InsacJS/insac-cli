...
class Logger {
  ...
  */
  constructor () {
    ...
    if (process.env.COLORS && process.env.COLORS === 'false') {
  ...
  */
  ...
  appExample (command) {
    const colors = this.colors
    const message = command ? `   $ insac ${command}` : ''
    process.stdout.write(` ${message}${colors.RESET}\n`)
  }
  ...
  appSuccess (before = '', word = '', after = '') {
    const colors = this.colors
    const COLOR1 = colors.NOTICE
    const COLOR2 = colors.INFO
    process.stdout.write(`  ${COLOR1}${before}${colors.RESET} ${COLOR2}${word}${colors.RESET} ${COLOR1}${after}${colors.RESET}\n`)
  }
}
