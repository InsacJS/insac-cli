...
module.exports = async function action (modelName, options) {
  ...
  const prod = options.production ? 'production/' : ''

  const MODULE_NAME    = options.module.toUpperCase()
  const MODULE_DIRNAME = cli.getModuleDirName(options.module)
  const SEED_PATH      = path.resolve(config.PATH.sources, `modules/${MODULE_DIRNAME}/seeders/${prod}${options.modelName}.seed.js`)
...
async function parseArgs (options) {
  ...
  options.production = options.production === true
